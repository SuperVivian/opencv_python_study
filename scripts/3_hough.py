...
class Hough:
    ...
    def new_mask(self):
        img = cv2.imread('../images/messi.jpg')
        mask = np.zeros(img.shape[:2], np.uint8)
        newmask = cv2.imread('../images/messi_mask.jpg', 0)
        mask[newmask == 0] = 0
        mask[newmask == 255] = 1
        rect = (50, 50, 450, 290)
        bgdModel = np.zeros((1, 65), np.float64)
        ...
        mask2 = np.where((mask == 2) | (mask == 0), 0, 1).astype('uint8')
        img = img * mask2[:, :, np.newaxis]
        plt.subplot(121), plt.imshow(newmask, 'gray'), plt.title('newmask'),plt.xticks([]), plt.yticks([])
        plt.subplot(122), plt.imshow(cv2.cvtColor(img,cv2.COLOR_BGR2RGB)), plt.title('result'), plt.xticks([]), plt.yticks([])
        plt.show()
